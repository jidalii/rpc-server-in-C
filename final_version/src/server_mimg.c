...
sem_t ** img_objs;
sem_t * img_objs_mutex;
sem_t * images_mutex;
sem_t * socket_notify; // proect connection socket
sem_t * image_count_mutex; // protect image_count 

// sem_t * img_arr; // protect img_order array
sem_t * img_orders_mutex; // protect variables of img_order
...
struct img_order ** img_orders = NULL;
...
void register_new_image(int conn_socket, struct request * req)
{
	...
	sem_wait(socket_notify);
	struct image * new_img = recvImage(conn_socket);
	sem_post(socket_notify);
	/* Store its pointer at the end of the global array */
	sem_wait(images_mutex);
	images[image_count - 1] = new_img;
	sem_post(images_mutex);
	
	img_objs = realloc(img_objs, image_count * sizeof(sem_t *));

	sem_t * img_mutex = (sem_t *)malloc(sizeof(sem_t));
	sem_init(img_mutex, 0, 1);

	sem_wait(img_objs_mutex);
	img_objs[image_count-1] = img_mutex;
	sem_post(img_objs_mutex);
	
	// update img_orders
	img_orders = realloc(img_orders, image_count * sizeof(sem_t));
	struct img_order * img_order = (struct img_order *)malloc(sizeof(struct img_order));
	img_order->current_op = 0;
	img_order->arrived = 0;
	img_orders[image_count-1] = img_order;
...
int worker_main (void * arg)
{
	...
	while (!params->worker_done) {
		...
		// ! protect arrived variable of img_order
		sem_wait(img_orders_mutex);
		my_arr = img_orders[img_id]->arrived;
		img_orders[img_id]->arrived++;
		sem_post(img_orders_mutex);

		// ! ensure ordering
		sem_wait(img_objs[img_id]);
		while(my_arr != img_orders[img_id]->current_op) {
			sem_post(img_objs[img_id]);
			sem_wait(img_objs[img_id]);
		}
		img_orders[img_id]->current_op++;
		...
		if (req.request.img_op != IMG_RETRIEVE) {
			if (req.request.overwrite) {
				...
				// sem_wait(image_count_mutex);
				img_id = image_count++;
				/* Reallocate array of image pointers */
				sem_wait(images_mutex);
				images = realloc(images, image_count * sizeof(struct image *));
				// sem_post(image_count_mutex);
				/* Store its pointer at the end of the global array */
				images[img_id] = img;
				sem_post(images_mutex);
			}
...
int main (int argc, char ** argv) {
	...
	socket_notify = (sem_t *)malloc(sizeof(sem_t));
	images_mutex = (sem_t *)malloc(sizeof(sem_t));
	retval = sem_init(socket_notify, 0, 1);
	if (retval < 0) {
		ERROR_INFO();
		perror("Unable to initialize socket notify");
		return EXIT_FAILURE;
	}

	img_objs_mutex = (sem_t *)malloc(sizeof(sem_t));
	sem_init(img_objs_mutex, 0, 1);

	// img_arr = (sem_t *)malloc(sizeof(sem_t));
	// retval = sem_init(img_arr, 0, 1);
	// if (retval < 0) {
	// 	ERROR_INFO();
	// 	perror("Unable to initialize img_arr");
	// 	return EXIT_FAILURE;
	// }

	retval = sem_init(images_mutex, 0, 1);
	if (retval < 0) {
		ERROR_INFO();
		perror("Unable to initialize images_mutex");
		return EXIT_FAILURE;
	}

	/* Initilize img_orders_mutex */
	img_orders_mutex = (sem_t *)malloc(sizeof(sem_t));
	sem_init(img_orders_mutex, 0, 1);
	/* Initilize tmage_count_mutex */
	image_count_mutex = (sem_t *)malloc(sizeof(sem_t));
	sem_init(image_count_mutex, 0, 1);
	...
	free(queue_mutex);
	free(queue_notify);
	free(image_count_mutex);
	free(img_orders_mutex);
