...
static struct image ** image_arr=NULL;
static uint64_t image_arr_count = 0;
...
int worker_main (void * arg)
{
	...
	while (!params->worker_done) {
		...
		struct image * img = image_arr[req.request.img_id];	
		...
		image_arr[req.request.img_id] = img;
		...
		if(req.request.img_op == IMG_RETRIEVE) {
			sendImage(image_arr[req.request.img_id], params->conn_socket);
		}
		// clock_gettime(CLOCK_MONOTONIC, &send_timestamp);
		/* IMPLEMENT ME! Print out the post-processing status report. */
		sync_printf("T%d R%lu:%f,%s,%d,%ld,%ld,%f,%f,%f\n",
			    params->worker_id,
                req.request.req_id,
		    ...
		    struct worker_params * common_params)
{
	...
	if (cmd == WORKERS_START) {
		...
		for (i = 0; i < worker_count; ++i) {
			...
			// worker_params[i]->image_arr = common_params->image_arr;
		}
	...
	do {
		...
		if (in_bytes > 0) {
			...
			if(req->request.img_op == IMG_REGISTER) {
				// set up resp
				// struct image * recv_image = malloc(sizeof(struct image));
				// recv_image = recvImage(conn_socket);
				// recv_image->img = recvImage(conn_socket);
				clock_gettime(CLOCK_MONOTONIC, &req->start_timestamp);
				struct image * recv_image = recvImage(conn_socket);

				if(recv_image != NULL){
					// recv and store img
					image_arr_count++;
					image_arr = (struct image **)realloc(image_arr, image_arr_count * sizeof(struct image *));
					
					if (image_arr == NULL) { 
						free(image_arr);
						perror("Unable to reallocate memory for image array.\n");
						break;
					}
					
					// add img to image_arr
					image_arr[image_arr_count-1] = recv_image;
					// set up img_id
					resp.img_id = image_arr_count-1;
					resp.ack = RESP_COMPLETED;
				}
				else {
					perror("Image received is NULL.\n");
					break;
				}
				resp.req_id = req->request.req_id;
				...
				// printf("handle_connection: after add to queue\n");
